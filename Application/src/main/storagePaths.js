...
function getStableUserDataRoot() {
  return path.join(app.getPath("appData"), STABLE_USER_DATA_NAME);
}

function getMigrationSourceRoots() {
  const appDataRoot = app.getPath("appData");
  const stableUserDataRoot = normalizePath(getStableUserDataRoot());
  const currentUserDataRoot = normalizePath(app.getPath("userData"));
  const sources = [];

  if (currentUserDataRoot !== stableUserDataRoot) {
    sources.push(app.getPath("userData"));
  }
